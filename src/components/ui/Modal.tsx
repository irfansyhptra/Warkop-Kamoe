...
const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="rounded-3xl border border-amber-200/20 bg-gradient-to-br from-amber-900/40 to-amber-800/30 backdrop-blur-2xl shadow-2xl">
          {/* Header */}
          {(title || showCloseButton) && (
            <div className="flex items-center justify-between p-6 border-b border-amber-200/20">
              {title && (
                <h3 className="text-xl font-bold text-amber-50">{title}</h3>
              )}
              {showCloseButton && (
                <button
                  onClick={onClose}
